...
- **StatefulSet**: 已验证完成。控制器支持 `volumeClaimTemplates`（动态创建 PVC）以及基于 `OrderedReady` 或 `Parallel` 策略生成稳定标识和主机名的 Pod。增加 `kubectl describe statefulset`。
- **Ingress**: 新增。已实现概念、解析校验和可视化拓扑中的虚拟流量路径（Client -> Ingress -> Service -> Pod）。
...
- 教学简化：Ingress 流量在拓扑图上仅作为一种连线视觉反馈存在，未在底层启用真正的反向代理引擎和 DNS 解析验证，只供教学理解 Ingress -> Service 的逻辑绑定。
