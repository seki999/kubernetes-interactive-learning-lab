  ...
  return pods
    .filter(
      (pod) =>
        ...
        !['Succeeded', 'Failed', 'Terminating'].includes(pod.status.phase)
    )
    .reduce(
      (sum, pod) => {
...
function expressionMatches(
  ...
  if (expression.operator === 'In')
    return value !== undefined && (expression.values ?? []).includes(value)
...
function nodeAffinityMatches(pod: Pod, node: Node): boolean {
  ...
  return terms.some((term) =>
    term.matchExpressions.every((expression) => expressionMatches(labels, expression))
  )
}
...
function podsInDomain(
  ...
  const nodeNames = new Set(
    nodes
      .filter((candidate) => candidate.metadata.labels?.[term.topologyKey] === domain)
...
function affinityTermsPass(
  terms: PodAffinityTerm[] | undefined,
  mode: 'affinity' | 'anti',
  node: Node,
  nodes: Node[],
  pods: Pod[],
...
function topologyPass(pod: Pod, node: Node, nodes: Node[], pods: Pod[]): boolean {
  return (pod.spec.topologySpreadConstraints ?? []).every((constraint) => {
    ...
    const domains = [
      ...new Set(
        nodes
          .map((item) => item.metadata.labels?.[constraint.topologyKey])
          .filter(Boolean)
      ),
    ]
    ...
    const counts = domains.map((domain) => {
      const names = new Set(
        nodes
          .filter((item) => item.metadata.labels?.[constraint.topologyKey] === domain)
          .map((item) => item.metadata.name)
      )
      const count = pods.filter(
        (item) =>
          Boolean(item.status.nodeName && names.has(item.status.nodeName)) &&
          labelsMatch(item.metadata.labels, constraint.labelSelector?.matchLabels)
      ).length
...
function evaluate(
  ...
  const podAffinity = affinityTermsPass(
  ...
  const podAntiAffinity = affinityTermsPass(
  ...
  const checks = [
    check(
      ...
      schedulable,
      '节点 Ready 且允许调度',
      node.spec.unschedulable ? '节点已 cordon，不可调度' : '节点状态不是 Ready'
    ),
    check(
      'NodeResourcesFit',
      resourcesFit,
      `CPU 满足（剩余 ${freeCpu}m/需要 ${requested.cpu}m），内存满足（剩余 ${Math.round(freeMemory)}Mi/需要 ${Math.round(requested.memory)}Mi）`,
      requested.cpu > freeCpu
        ? `CPU 资源不足（剩余 ${freeCpu}m，需要 ${requested.cpu}m）`
        : `内存资源不足（剩余 ${Math.round(freeMemory)}Mi，需要 ${Math.round(requested.memory)}Mi）`
    ),
    ...
    check(
      ...
      taints,
      'Taint/Toleration 匹配',
      '存在阻止调度的 Taint，Pod 没有对应 Toleration'
    ),
    ...
    check(
      ...
      podAffinity,
      'Pod Affinity 匹配',
      'Pod Affinity 要求的 Pod 不在同一拓扑域'
    ),
    check(
      'PodAntiAffinity',
      podAntiAffinity,
      'Pod Anti-Affinity 匹配',
      'Pod Anti-Affinity 禁止的 Pod 已在同一拓扑域'
    ),
    check(
      'TopologySpread',
      spread,
      '拓扑分散约束满足',
      '放置后会超过 topologySpreadConstraints.maxSkew'
    ),
  ]
  ...
  if (rejectionReasons.length)
    return { nodeName: node.metadata.name, feasible: false, checks, rejectionReasons }

  const cpuHeadroom = allocCpu > 0 ? freeCpu / allocCpu : 0
  const memoryHeadroom = allocMemory > 0 ? freeMemory / allocMemory : 0
  const rawScore =
    50 +
    Math.max(0, Math.min(1, cpuHeadroom)) * 25 +
    Math.max(0, Math.min(1, memoryHeadroom)) * 25
  ...
  return {
    ...
    checks,
    score,
    scoreExplanation: `标准化得分: ${score} (原始总分: ${rawScore.toFixed(2)})\n基础分: 50\nCPU 评分: ${(Math.max(0, Math.min(1, cpuHeadroom)) * 25).toFixed(2)}\n内存评分: ${(Math.max(0, Math.min(1, memoryHeadroom)) * 25).toFixed(2)}`,
    rejectionReasons: [],
  }
}

export function explainSchedulingDecision(
  pod: Pod,
  nodes: Node[],
  existingPods: Pod[]
): SchedulerDecision {
  ...
  const feasible = candidates
    ...
    .sort(
      (a, b) => (b.score ?? 0) - (a.score ?? 0) || a.nodeName.localeCompare(b.nodeName)
    )
  ...
  return {
    ...
    failureDetails: decision.candidates
      ...
      .map((item) => ({
        ...
        reason: item.rejectionReasons.join('；'),
      })),
    decision,
  }
}
