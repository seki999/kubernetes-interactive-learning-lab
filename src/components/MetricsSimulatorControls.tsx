  ...
  return (
    <section className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/30">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono bg-amber-100 text-amber-800 px-2 py-0.5 rounded dark:bg-amber-900 dark:text-amber-200">
          模型: {profile.trafficModel}
        </span>
      </div>
      ...
      {hpa && (
        <div className="mt-2 rounded-md border border-amber-300 bg-white p-2 text-xs dark:border-amber-800 dark:bg-slate-900">
          ...
          <div className="mt-1 text-slate-600 dark:text-slate-300 grid grid-cols-2 gap-2">
            <div>当前副本：{hpa.status.currentReplicas}</div>
            <div>期望副本：{hpa.status.desiredReplicas}</div>
            <div>下限：{hpa.spec.minReplicas ?? 1}</div>
            <div>上限：{hpa.spec.maxReplicas}</div>
          </div>
          {hpa.status.message && (
            <p className="mt-2 text-amber-700 dark:text-amber-400 font-semibold">
              状态：{hpa.status.message}
            </p>
          )}
          {hpa.status.calculationDetails && hpa.status.calculationDetails.length > 0 && (
            <div className="mt-2 p-2 bg-slate-50 dark:bg-slate-800 rounded">
              <p className="font-medium mb-1">HPA 决策详情：</p>
              <ul className="list-disc list-inside pl-4 space-y-1">
                {hpa.status.calculationDetails.map((detail, i) => (
                  <li key={i}>{detail}</li>
                ))}
              </ul>
            </div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setTrafficModel(namespace, name, 'increasing')}
        ...
        <button
          type="button"
          onClick={() => setTrafficModel(namespace, name, 'decreasing')}
        ...
        <button
          type="button"
          onClick={() => {
            setTrafficModel(namespace, name, 'steady')
            resetLoadProfile(namespace, name)
          }}
          className="rounded border border-slate-300 px-2 py-1 text-xs dark:border-slate-700"
        >
          重置为稳态
      ...
      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
        使用率是相对于容器 resources.requests
        的百分比，由用户在这里显式设置或通过自动流量模型动态改变；kubectl top 和 HPA
        都读取同一份数据。
      </p>
    </section>
  )
